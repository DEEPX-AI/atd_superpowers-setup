...
**GitHub Copilot**은 회사·팀 단위 라이선스로 많이 운영됩니다. Copilot CLI는 Claude Code와 동급의 런타임 Hook을 지원하며(`.github/extensions/*/extension.mjs`), VS Code Copilot Chat도 셸 스크립트 기반 Hook(`.github/hooks/hooks.json`)을 지원합니다. 단, Superpowers 공식 지원은 없고 커뮤니티 포트에 의존합니다.
...
GitHub Copilot CLI 및 VS Code Copilot Chat Hook 설치 스크립트.
...
# 2단계: 프로젝트별 Hook 강제화
bash superpowers-copilot-hook/install-superpowers-copilot-cli-hooks.sh /path/to/project     # CLI
bash superpowers-copilot-hook/install-superpowers-copilot-vscode-hooks.sh /path/to/project  # VS Code Chat

# 제거 (Hook만, 전역 플러그인 유지)
bash superpowers-copilot-hook/install-superpowers-copilot-cli-hooks.sh uninstall /path/to/project
...
| `install-superpowers-copilot-cli-hooks.sh` | `superpowers-copilot-hook/` | ✅ | Copilot CLI Hook 강제화 |
| `install-superpowers-copilot-vscode-hooks.sh` | `superpowers-copilot-hook/` | ✅ | VS Code Copilot Chat Hook 강제화 |
